...
class Alignment(object):
    ...
    def __output_alignment__(self, lower, middle, upper, curr_graph, s_mod, t_mod, s, t, i, j, match_reward, mismatch_penalty, gap_opening_penalty, gap_extension_penalty):
        ...
        if curr_graph is lower:
            ...
            s_mod = s[i - 1] + s_mod
            t_mod = '-' + t_mod
            return self.output_alignment(lower, middle, upper, curr_graph, s_mod, t_mod, s, t, i - 1, j, match_reward, mismatch_penalty, gap_opening_penalty, gap_extension_penalty)
        elif curr_graph is upper:
            if j > 0 and upper[i][j] == middle[i][j - 1] - gap_opening_penalty:
                curr_graph = middle
            s_mod = '-' + s_mod
            t_mod = t[j - 1] + t_mod
            return self.output_alignment(lower, middle, upper, curr_graph, s_mod, t_mod, s, t, i, j - 1, match_reward, mismatch_penalty, gap_opening_penalty, gap_extension_penalty)
        else:
            if middle[i][j] == lower[i][j]:
                curr_graph = lower
                return self.output_alignment(lower, middle, upper, curr_graph, s_mod, t_mod, s, t, i, j, match_reward, mismatch_penalty, gap_opening_penalty, gap_extension_penalty)
                ...
                s_mod = s[i - 1] + s_mod
                t_mod = t[j - 1] + t_mod
                return self.output_alignment(lower, middle, upper, curr_graph, s_mod, t_mod, s, t, i - 1, j - 1, match_reward, mismatch_penalty, gap_opening_penalty, gap_extension_penalty)
    ...
    def __bottomAlignment__(self):
        ...
        n = self.db[self.seed + 1:]

        return self.affine_alignment(2, 3, 5, 2, m, n)
